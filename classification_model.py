...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, entity_mask, segment_ids, label_id):
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    entity_mask = get_entity_mask(tokens, tokenizer)
    ...
    if ex_index < 5:
        ...
        tf.logging.info("entity_masks: %s" % " ".join([str(x) for x in entity_mask]))
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["entity_mask"] = create_int_feature(feature.entity_mask)
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    ...
    name_to_features = {
        ...
        "entity_mask": tf.FixedLenFeature([3], tf.int64),
...
def create_model(bert_config, is_training, features,
    ...
    with tf.variable_scope("loss"):
        if is_training:
            # I.e., 0.1 dropout
            masked_output = tf.nn.dropout(masked_output, keep_prob=0.9)

        logits = tf.matmul(masked_output, output_weights, transpose_b=True)
